...
with tf.Session() as sess:
    # Run the initializer
    writer = tf.summary.FileWriter('./graphs/stack_autoencoder', sess.graph)
    sess.run(init)
    tf.summary.scalar("loss", loss)  # *********************************************
    summary_writer = tf.summary.merge_all()
    merged_summary_op = tf.summary.merge_all()
    summary_writer = tf.summary.FileWriter('./graphs/stack_loss', graph=tf.get_default_graph())  # ******************
    ...
    for i in range(1, num_steps + 1):
        ...
        c, summary = sess.run([loss, merged_summary_op], feed_dict={X: batch_x})  # ********************
